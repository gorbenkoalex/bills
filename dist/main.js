...
function parseReceipt(text) {
    const lines = text
        ...
        .filter(Boolean);
    const skipKeywords = /ukupno|za platiti|popust|kartic|gotovina|polog|pdv|napomena|artikl|cijena|suma|total|tax|change|payed|paid|summary|resume|resuma|bon/i;
    const itemMatches = [];
    const itemPatterns = [
    ...
    const cleanDescription = (desc) => desc.replace(/[^\p{L}\p{N}\s.'-]/gu, '').trim();
    ...
    const storeLine = headerSlice.find((line) => /plodine|market|store|shop|magazin|coop|kaufland|spar|lidl/i.test(line)) ||
    ...
    if (!grandTotal && itemMatches.length) {
        grandTotal = Number(itemMatches.reduce((sum, item) => sum + (Number.isFinite(item.total) ? item.total : 0), 0).toFixed(2));
    }
    return {
        storeName: storeLine,
